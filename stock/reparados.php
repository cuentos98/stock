...
<table>
        ...
        <?php
            ...
            $consulta = mysql_query("SELECT * FROM equipos WHERE ESTADO = 3");

            if ($consulta) {
                while ($registro = mysql_fetch_assoc($consulta)) {
                    echo "<tr>  <td><input type=\"button\" value=\"Eliminar\" onclick=\"location.href='eliminar.php?NROORDEN=".$registro["NROORDEN"]."'\">
                        <input type=\"button\" value=\"Entregar\" onclick=\"location.href='entregar.php?NROORDEN=".$registro["NROORDEN"]."'\">
                         
                    </td>";
                    echo "<td>".$registro["NROORDEN"]."</td>";
                    echo "<td>".$registro["FECHAFINALIZACION"]."</td>";
