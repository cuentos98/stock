...
<table>
        ...
        <?php
            ...
            $consulta = mysql_query("SELECT * FROM equipos WHERE ESTADO = 1");

            if ($consulta) {
                while ($registro = mysql_fetch_assoc($consulta)) {
                    echo "<tr>  <td><input type=\"button\" value=\"Eliminar\" onclick=\"location.href='eliminar.php?NROORDEN=".$registro["NROORDEN"]."'\">
                        <input type=\"button\" value=\"Confirmar\" onclick=\"location.href='confirmar.php?NROORDEN=".$registro["NROORDEN"]."'\">
                        <input type=\"button\" value=\"Rechazar\" onclick=\"location.href='rechazar.php?NROORDEN=".$registro["NROORDEN"]."'\">    
                    </td>";
                    echo "<td>".$registro["NROORDEN"]."</td>";
                    echo "<td>".$registro["FECHADIAGNOSTICO"]."</td>";
